        ...
        >
            ...
            <div className="App-content">
              <Switch>
                <Route
                  exact
                  path="/sources"
                  render={() => <CreditsView {...childProps} />}
                />
                <Route
                  exact
                  path="/explain"
                  render={() => <ExplainNumbersView {...childProps} />}
                />

                <Route
                  exact
                  path="/t"
                  render={() => <TableView {...childProps} filter="relevant" />}
                />

                <Route
                  exact
                  path="/"
                  render={() => <TableView {...childProps} filter="all" />}
                />
                <Route
                  exact
                  path="/all"
                ...
                <Route
                  exact
                  path="/euro"
                  render={() => <TableView {...childProps} filter="europe" />}
                />
                <Route
                  exact
                  path="/usa"
                  render={() => <TableView {...childProps} filter="usa" />}
                />
                <Route
                  exact
                  path="/latam"
                  render={() => <TableView {...childProps} filter="latam" />}
                />
                <Route
                  exact
                  path="/asia"
                  render={() => <TableView {...childProps} filter="asia" />}
                />
          ...
          {error && (
            <div className="App-error">
              <div>
                <h2>
                  <Trans i18nKey={"general.error_title"}>ERROR</Trans>
                </h2>

                <Trans i18nKey={"general.error_text"}>
                  <p>
                    Something unexpected happened. We suggest clicking the
                    button below to reset cached data and try loading the page
                    again.
                  </p>
                  <p>
                    If an error is still happening, reach out to{" "}
                    <a href="https://twitter.com/">@sd on twitter</a>.
                  </p>
                </Trans>
...
const PageHeader = ({ listRef, isMobile }) => {
  return (
    <header className="App-header">
      <h1
        onClick={() => listRef.current && listRef.current.scrollTo(0, 0)}
